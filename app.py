...
# -- Modeling -----------------------------------------------------------
    ...
    if st.button("Train & Evaluate"):
        ...
        if choice in options[:3]:
            ...
            Xt, Xv, yt, yv = train_test_split(Xp, y, test_size=test_pct/100, random_state=42, stratify=y)
            ...
            proc_xgb    = ColumnTransformer([("num", num_pipe, num_cols), ("cat", cat_pipe, cat_cols_x)])
            scale_w     = y_train.value_counts()[0] / y_train.value_counts()[1]
            xgb_clf     = XGBClassifier(use_label_encoder=False, eval_metric="logloss", scale_pos_weight=scale_w, random_state=42)
...
# -- Predict ------------------------------------------------------------
    ...
    if submitted:
        with st.spinner("⏳ Please wait while we process your prediction..."):
